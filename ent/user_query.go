...
type UserQuery struct {
	...
	withTournamentAdminRoles *TournamentAdminQuery
...
// QueryUserVotes chains the current query on the "user_votes" edge.
func (_q *UserQuery) QueryUserVotes() *UserVoteQuery {
	query := (&UserVoteClient{config: _q.config}).Query()
...
// QueryCreatedVotes chains the current query on the "created_votes" edge.
func (_q *UserQuery) QueryCreatedVotes() *VoteQuery {
...
// QueryTournamentAdminRoles chains the current query on the "tournament_admin_roles" edge.
func (_q *UserQuery) QueryTournamentAdminRoles() *TournamentAdminQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTournamentAdminRoles: _q.withTournamentAdminRoles.Clone(),
...
// WithUserVotes tells the query-builder to eager-load the nodes that are connected to
// the "user_votes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUserVotes(opts ...func(*UserVoteQuery)) *UserQuery {
	query := (&UserVoteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserVotes = query
	return _q
}

// WithCreatedVotes tells the query-builder to eager-load the nodes that are connected to
// the "created_votes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCreatedVotes(opts ...func(*VoteQuery)) *UserQuery {
	query := (&VoteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithTournamentAdminRoles tells the query-builder to eager-load the nodes that are connected to
// the "tournament_admin_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithTournamentAdminRoles(opts ...func(*TournamentAdminQuery)) *UserQuery {
	query := (&TournamentAdminClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTournamentAdminRoles = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [9]bool{
			...
			_q.withTournamentAdminRoles != nil,
		}
	)
	...
	if query := _q.withUserVotes; query != nil {
		if err := _q.loadUserVotes(ctx, query, nodes,
			func(n *User) { n.Edges.UserVotes = []*UserVote{} },
			func(n *User, e *UserVote) { n.Edges.UserVotes = append(n.Edges.UserVotes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCreatedVotes; query != nil {
	...
	if query := _q.withTournamentAdminRoles; query != nil {
		if err := _q.loadTournamentAdminRoles(ctx, query, nodes,
			func(n *User) { n.Edges.TournamentAdminRoles = []*TournamentAdmin{} },
			func(n *User, e *TournamentAdmin) {
				n.Edges.TournamentAdminRoles = append(n.Edges.TournamentAdminRoles, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadUserVotes(ctx context.Context, query *UserVoteQuery, nodes []*User, init func(*User), assign func(*User, *UserVote)) error {
	...
	query.withFKs = true
	query.Where(predicate.UserVote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserVotesColumn), fks...))
	}))
...
func (_q *UserQuery) loadCreatedVotes(ctx context.Context, query *VoteQuery, nodes []*User, init func(*User), assign func(*User, *Vote)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Vote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CreatedVotesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadTournamentAdminRoles(ctx context.Context, query *TournamentAdminQuery, nodes []*User, init func(*User), assign func(*User, *TournamentAdmin)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TournamentAdmin(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TournamentAdminRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_tournament_admin_roles
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_tournament_admin_roles" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_tournament_admin_roles" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
