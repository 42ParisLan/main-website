...
func (_q *TeamQuery) QueryTournament() *TournamentQuery {
...
// QueryRankGroup chains the current query on the "rank_group" edge.
func (_q *TeamQuery) QueryRankGroup() *RankGroupQuery {
...
func (_q *TeamQuery) QueryInvitations() *InvitationQuery {
...
func (_q *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withRankGroup:   _q.withRankGroup.Clone(),
...
func (_q *TeamQuery) WithTournament(opts ...func(*TournamentQuery)) *TeamQuery {
	query := (&TournamentClient{config: _q.config}).Query()
...
// WithRankGroup tells the query-builder to eager-load the nodes that are connected to
// the "rank_group" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TeamQuery) WithRankGroup(opts ...func(*RankGroupQuery)) *TeamQuery {
	query := (&RankGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRankGroup = query
	return _q
}
...
func (_q *TeamQuery) WithInvitations(opts ...func(*InvitationQuery)) *TeamQuery {
	query := (&InvitationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			_q.withCreator != nil,
			_q.withMembers != nil,
			_q.withRankGroup != nil,
			_q.withInvitations != nil,
		}
	)
	if _q.withTournament != nil || _q.withCreator != nil || _q.withRankGroup != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTournament; query != nil {
		if err := _q.loadTournament(ctx, query, nodes, nil,
			func(n *Team, e *Tournament) { n.Edges.Tournament = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withRankGroup; query != nil {
		if err := _q.loadRankGroup(ctx, query, nodes, nil,
			func(n *Team, e *RankGroup) { n.Edges.RankGroup = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInvitations; query != nil {
		if err := _q.loadInvitations(ctx, query, nodes,
			func(n *Team) { n.Edges.Invitations = []*Invitation{} },
			func(n *Team, e *Invitation) { n.Edges.Invitations = append(n.Edges.Invitations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadTournament(ctx context.Context, query *TournamentQuery, nodes []*Team, init func(*Team), assign func(*Team, *Tournament)) error {
...
func (_q *TeamQuery) loadRankGroup(ctx context.Context, query *RankGroupQuery, nodes []*Team, init func(*Team), assign func(*Team, *RankGroup)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Team)
	for i := range nodes {
	...
	query.Where(rankgroup.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *TeamQuery) loadInvitations(ctx context.Context, query *InvitationQuery, nodes []*Team, init func(*Team), assign func(*Team, *Invitation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
