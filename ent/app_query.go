...
func (_q *AppQuery) QueryOwner() *UserQuery {
...
func (_q *AppQuery) QueryConsents() *ConsentQuery {
...
func (_q *AppQuery) WithOwner(opts ...func(*UserQuery)) *AppQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AppQuery) WithConsents(opts ...func(*ConsentQuery)) *AppQuery {
	query := (&ConsentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AppQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*App, error) {
	...
	if query := _q.withConsents; query != nil {
		if err := _q.loadConsents(ctx, query, nodes,
			func(n *App) { n.Edges.Consents = []*Consent{} },
			func(n *App, e *Consent) { n.Edges.Consents = append(n.Edges.Consents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AppQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*App, init func(*App), assign func(*App, *User)) error {
...
func (_q *AppQuery) loadConsents(ctx context.Context, query *ConsentQuery, nodes []*App, init func(*App), assign func(*App, *Consent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(consent.FieldApplicationID)
	}
	query.Where(predicate.Consent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(app.ConsentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *AppQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOwner != nil {
			_spec.Node.AddColumnOnce(app.FieldOwnerID)
		}
	}
