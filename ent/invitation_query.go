...
func (_q *InvitationQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Team)) error {
	...
	for i := range nodes {
		if nodes[i].invitation_team == nil {
			continue
		}
		fk := *nodes[i].invitation_team
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invitation_team" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadInvitee(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	...
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
		if nodes[i].invitation_invitee == nil {
			continue
		}
		fk := *nodes[i].invitation_invitee
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invitation_invitee" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
