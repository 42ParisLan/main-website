...
func (_q *InvitationQuery) QueryTeam() *TeamQuery {
...
func (_q *InvitationQuery) WithTeam(opts ...func(*TeamQuery)) *InvitationQuery {
	query := (&TeamClient{config: _q.config}).Query()
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if _q.withTeam != nil || _q.withInvitee != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTeam; query != nil {
		if err := _q.loadTeam(ctx, query, nodes, nil,
			func(n *Invitation, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *InvitationQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Team)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
		if nodes[i].team_invitations == nil {
			continue
		}
		fk := *nodes[i].team_invitations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_invitations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadInvitee(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	...
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
		if nodes[i].user_received_invitations == nil {
			continue
		}
		fk := *nodes[i].user_received_invitations
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_received_invitations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
